...
def handle_join(data):
    ...
    # Daha önce katılmışsa aynı renkten devam
...
def handle_game_over(data):
    ...
    # Oyuncuları temizle (oda kalsın, tekrar oynanabilir)
    if room in rooms:
        rooms[room]["players"] = {}
...
# WebSocket: Oyuncu ayrıldıysa (sayfayı kapattıysa vs.)
    ...
    if username:
        for room, data in rooms.items():
            if username in data["players"]:
                ...
                emit("player_joined", data["players"], room=room)
                break

        del usernames[sid]

# Uygulama başlatma (Render için gunicorn kullanılacaksa burası çalışmaz)
